...
export default function QuantDashboard() {
  return (
    <div className="flex-1 p-2 bg-[#050505] text-neutral-300 font-mono text-xs overflow-y-auto">
      <div className="grid grid-cols-12 gap-2 max-w-[2000px] mx-auto">
        ...
        <div className="col-span-12 flex justify-between items-center px-2 py-1 bg-[#0a0a0a] border border-neutral-800 rounded-sm">
           <div className="flex space-x-6">
             <div className="flex items-center space-x-2">
               <div className="w-2 h-2 rounded-full bg-[#06F7C9] animate-pulse" />
               <span className="text-[#06F7C9] font-semibold tracking-wider">LIVE LAB</span>
             </div>
           ...
           <div className="flex space-x-4 items-center">
             ...
             <div className="text-right">
               ...
               <div className="text-[#C026D3] font-bold">$14.5M</div>
             </div>
        ...
        <div className="col-span-12 lg:col-span-8 flex flex-col gap-2">
          ...
          <Card className="min-h-[400px] md:h-64 flex flex-col">
            <CardHeader>
              <CardTitle className="flex items-center space-x-2"><Crosshair size={14} className="text-[#C026D3]"/> <span>Signal Fusion Matrix</span></CardTitle>
              ...
              <div className="w-full md:w-1/3 border-b md:border-b-0 md:border-r border-neutral-800 p-4 shrink-0">
                <div className="mb-4">
                  ...
                  <div className="w-full bg-neutral-900 h-1">
                    <div className="bg-[#06F7C9] h-1" style={{ width: '84%' }} />
                  </div>
                </div>
                  ...
                  {signalConfidence.map((s, idx) => (
                    <div key={idx} className="flex justify-between items-center text-[10px]">
                      ...
                      <div className="flex-1 px-2">
                        <div className="w-full bg-neutral-900 h-1">
                          <div className={s.score > 80 ? 'bg-[#06F7C9] h-1' : s.score > 70 ? 'bg-amber-500 h-1' : 'bg-red-500 h-1'} style={{ width: `${s.score}%` }} />
                        </div>
                      </div>
            ...
            <CardContent className="p-0 bg-neutral-950 flex-1 relative overflow-hidden">
              ...
              <div className="relative z-10 flex h-full w-full min-w-[750px] items-center justify-between px-6 py-6 overflow-x-auto">
                ...
                <div className="flex flex-col space-y-8 w-36 relative">
                  ...
                  <div className="bg-blue-900/20 border border-[#C026D3]/50 p-3 rounded-lg text-center shadow-[0_0_15px_rgba(59,130,246,0.15)] hover:shadow-[0_0_25px_rgba(59,130,246,0.3)] transition-all cursor-default" title="Technical patterns and order flow analysis">
                    ...
                    <div className="absolute -top-2 -right-2 bg-[#C026D3]/20 text-[#C026D3] text-[8px] px-1.5 py-0.5 rounded border border-[#C026D3]/50">Conf: 89%</div>
                    <div className="text-[#C026D3] font-bold mb-1 flex items-center justify-center space-x-1"><Cpu size={10}/><span>BETAFISH</span></div>
                    <div className="text-[10px] text-blue-200/70">Analyzing Flow</div>
                    <div className="mt-2 w-full bg-blue-950 rounded-full h-1"><div className="bg-[#C026D3] h-1 rounded-full w-[70%] animate-pulse"></div></div>
                  </div>

                  <div className="bg-blue-900/20 border border-[#C026D3]/50 p-3 rounded-lg text-center shadow-[0_0_15px_rgba(59,130,246,0.15)] hover:shadow-[0_0_25px_rgba(59,130,246,0.3)] transition-all cursor-default" title="Macro trends and sentiment analysis">
                    <div className="absolute -left-2 top-1/2 flex h-2 w-2 -translate-y-1/2">
                      <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#C026D3] opacity-75"></span>
                      <span className="relative inline-flex rounded-full h-2 w-2 bg-[#C026D3]"></span>
                    </div>
                    <div className="absolute -top-2 -right-2 bg-[#C026D3]/20 text-[#C026D3] text-[8px] px-1.5 py-0.5 rounded border border-[#C026D3]/50">Conf: 72%</div>
        ...
        <div className="col-span-12 lg:col-span-4 flex flex-col gap-2">
          ...
          <Card className="min-h-[220px] md:h-auto flex flex-col">
            <CardHeader className="bg-neutral-950 border-b border-neutral-900">
              <CardTitle className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  <Activity size={14} className="text-[#06F7C9]"/> 
                  <span className="text-sm text-gray-300">Perpetual Funding Rates</span>
                </div>
                <div className="text-[10px] text-neutral-500 bg-neutral-900 px-2 py-1 rounded">MIROFISH</div>
              </CardTitle>
            </CardHeader>
            <CardContent className="p-0 text-neutral-400 font-mono flex-1">
              <div className="h-full overflow-y-auto overflow-x-auto">
                <table className="w-full text-left min-w-[340px]">
                  <thead className="bg-[#0a0a0a] text-neutral-500 sticky top-0 border-b border-neutral-800">
                    <tr>
                      <th className="px-4 py-2 font-normal">ASSET</th>
                      <th className="px-4 py-2 font-normal text-right">8H RATE</th>
                      <th className="px-4 py-2 font-normal text-right hidden sm:table-cell">ANNUAL</th>
                      <th className="px-4 py-2 font-normal text-center">PRED</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-neutral-900/50 whitespace-nowrap">
                    {[
                      { sym: 'BTC-PERP', rate: 0.0125, ann: 13.68, bias: 'SHORT' },
                      { sym: 'ETH-PERP', rate: 0.0150, ann: 16.42, bias: 'SHORT' },
                      { sym: 'SOL-PERP', rate: 0.0450, ann: 49.27, bias: 'SHORT' },
                      { sym: 'DOGE-PERP', rate: -0.0100, ann: -10.95, bias: 'LONG' },
                      { sym: 'AVAX-PERP', rate: 0.0210, ann: 22.99, bias: 'SHORT' },
                    ].map((row, i) => (
                      <tr key={i} className="hover:bg-neutral-900/50">
                        <td className="px-4 py-2 font-bold text-white">{row.sym}</td>
                        <td className={`px-4 py-2 text-right ${row.rate > 0 ? 'text-amber-400' : 'text-[#06F7C9]'}`}>{row.rate > 0 ? '+' : ''}{row.rate.toFixed(4)}%</td>
                        <td className={`px-4 py-2 text-right hidden sm:table-cell ${row.ann > 0 ? 'text-amber-400' : 'text-[#06F7C9]'}`}>{row.ann > 0 ? '+' : ''}{row.ann.toFixed(2)}%</td>
                        <td className="px-4 py-2 text-center">
                           <span className={`px-2 py-0.5 rounded text-[9px] font-bold ${row.bias === 'LONG' ? 'bg-[#06F7C9]/20 text-[#06F7C9]' : 'bg-red-500/20 text-red-400'}`}>{row.bias}</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          <Card className="h-64 bg-red-950/10 border-red-900/20">
            ...
            <CardContent className="space-y-4 pt-4">
               ...
               <div>
                 ...
                 <div className="w-full bg-neutral-900 h-1">
                   <div className="bg-[#C026D3] h-1 w-[14%]" />
                 </div>
               </div>
               
               <div className="pt-2">
                 <div className="text-xs text-neutral-500 mb-2">Automated Rules Enforced:</div>
                 <div className="flex items-center space-x-2 text-[#06F7C9]/80">
                   <GitCommit size={12}/> <span>Hard Stop-Loss at 2.5% Position Risk</span>
                 </div>
                 <div className="flex items-center space-x-2 text-[#06F7C9]/80 mt-1">
