...
export function Sidebar({ currentTab, setTab }: { currentTab: string, setTab: (t: string) => void }) {
  ...
  return (
    <div className="w-64 bg-[#050505] border-r border-[#1a1a1a] flex flex-col h-screen shrink-0 relative z-20">
      <div className="p-5 flex items-center gap-3 border-b border-[#1a1a1a]">
         <div className="w-8 h-8 rounded bg-gradient-to-br from-[#06F7C9] to-[#006B4F] flex items-center justify-center font-bold text-black shadow-[0_0_15px_rgba(252,213,53,0.3)]">
           N
         </div>
         <div className="flex flex-col">
           <span className="font-['Syne'] font-bold text-sm tracking-wide bg-gradient-to-r from-white to-[#06F7C9] bg-clip-text text-transparent">NEXUS O.S.</span>
          ...
          return (
            <button
              ...
              className={clsx(
                ...
                active ? "bg-[#06F7C9]/10 border border-[#06F7C9]/20" : "hover:bg-white/5 border border-transparent"
              )}
            >
              <Icon size={18} className={active ? "text-[#06F7C9]" : "text-gray-500 group-hover:text-gray-300"} />
              <div className="flex flex-col flex-1">
                 <span className={clsx("text-xs font-semibold tracking-tight", active ? "text-[#06F7C9]" : "text-gray-300")}>{item.label}</span>
                 <span className="text-[9px] text-gray-500">{item.subtitle}</span>
              </div>
              {active && <div className="w-1.5 h-1.5 rounded-full bg-[#06F7C9] shadow-[0_0_5px_#06F7C9]" />}
            </button>
          )
        })}
      </div>
