...
export function BottomNav({ 
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 bg-[#050505]/90 backdrop-blur-md border-t border-white/10 z-50 px-2 pb-safe">
          ...
          return (
            <button
              ...
              className={clsx(
                ...
                active ? "text-[#06F7C9]" : "text-gray-500 hover:text-gray-300"
              )}
            >
              <Icon size={20} className={active ? "drop-shadow-[0_0_8px_rgba(252,213,53,0.5)]" : ""} />
...
export function MobileMenuDrawer({ 
  ...
  return (
    <div className="md:hidden fixed inset-0 z-[60] flex flex-col justify-end">
      <div className="absolute inset-0 bg-[#050505]/60 backdrop-blur-sm" onClick={onClose} />
      
      <div className="relative bg-[#1E2329] border-t border-white/10 rounded-t-2xl p-4 flex flex-col gap-2 max-h-[80vh] overflow-y-auto animate-in slide-in-from-bottom-full duration-200">
        ...
        <button onClick={() => handleSelect('trading')} className="flex items-center gap-3 p-4 bg-white/5 rounded-xl text-left border border-white/5 active:bg-white/10">
          <Network size={20} className="text-[#C026D3]" />
