    ...
    <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <button 
          ...
          style={{ padding: '6px 12px', background: 'rgba(252,213,53,0.1)', border: '1px solid rgba(252,213,53,0.3)', color: '#06F7C9', borderRadius: '8px', cursor: 'pointer', fontSize: '12px', fontFamily: "'DM Mono', monospace" }}
        >
      ...
      {showAddModel && (
        <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(252,213,53,0.3)', borderRadius: '12px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <div style={{ fontSize: '14px', fontWeight: 'bold', color: '#06F7C9' }}>Yeni Yerel Model Tanımla</div>
          ...
          <button 
            ...
            style={{ padding: '8px', background: 'linear-gradient(135deg, #06F7C9, #009E70)', border: 'none', color: '#000', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold' }}
          >
          ...
          {models.filter(m => m.id !== 'default-local').map(m => (
            <div key={m.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(0,0,0,0.3)', padding: '8px 12px', borderRadius: '6px', border: '1px solid rgba(255,255,255,0.1)' }}>
              ...
              <button 
                ...
                style={{ background: 'rgba(255,77,109,0.2)', border: '1px solid rgba(255,77,109,0.4)', color: '#F6465D', padding: '4px 10px', borderRadius: '4px', fontSize: '11px', cursor: 'pointer' }}
              >
                Sil
              </button>
            </div>
      ...
      {agents.map(agent => (
        <div key={agent.id} style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
            ...
            <div 
              style={{ padding: '4px 12px', borderRadius: '12px', background: agent.enabled ? 'rgba(0,255,0,0.2)' : 'rgba(255,0,0,0.2)', cursor: 'pointer', fontSize: '11px', fontWeight: 'bold', color: agent.enabled ? '#06F7C9' : '#F6465D' }}
            ...
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
              ...
              {agent.modelAssignmentType === 'assigned' && agent.assignedModelId && agent.assignedModelId !== 'default-local' && (
                <button 
                  ...
                  style={{ background: 'rgba(255,77,109,0.2)', border: '1px solid rgba(255,77,109,0.4)', color: '#F6465D', padding: '6px 10px', borderRadius: '4px', fontSize: '11px', cursor: 'pointer', whiteSpace: 'nowrap' }}
                >
                  Seçili Modeli Sil
                </button>
              )}
          ...
          <div style={{ marginTop: '12px' }}>
            ...
            <input 
              ...
              style={{ width: '100%', marginTop: '8px', accentColor: '#06F7C9' }}
            />
          </div>
        </div>
      ))}
